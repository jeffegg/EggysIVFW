...
console_formatter = logging.Formatter('%(asctime)s – %(levelname)s: %(message)s’, datefmt=’%d/%m/%Y %I:%M:%S %p')
consoleHandler.setFormatter(console_formatter)

fileHandler = logging.FileHandler(filename='FWUpdater_{:%Y-%m-%d_%H-%M-%S}.log'.format(datetime.now()))
...
class FWUpdater:
    ...
    def fw_updater(self, alt_fw_path="", rs485_port="", skip_valve_enum=False, enumerate_timeout=30, valve_uuid="", valve_type="", valve_address=0x0, num_retries=10):
        ...
        while(1):
            ...
            choice = input('Flash Another valve? (q)uit or any key: ')
            if choice.lower() == 'q':
                break
    ...
    def goto_fw_update_mode_eggy(self, uuid: bytearray, address: int, enumerate_timeout=15, num_reset_before_quit=10):
        print(f"Opening serial port: {self._rs485_loc}")
        ser = serial.Serial(self._rs485_loc, 9600)
        ...
        fw_mode_packet += bytearray([0x1])
        self._calculate_and_append_checksum(fw_mode_packet)
        i = num_reset_before_quit
        for i in range(num_reset_before_quit):
            print(f'Forcing valve address: {hex(address)} uuid: {uuid.hex("-")} to reset')
            ser.write(fw_mode_packet)
            ser.flush()
            time.sleep(0.5)
            ser.timeout = enumerate_timeout
            pentair_packet = bytearray([0xFF, 0x0, 0xFF, 0xA5, 0x1, 0x10, 0xC, 0x52, 0x8])
            pentair_packet += bytearray([0x0, 0x80])
            pentair_packet += uuid
            self._calculate_and_append_checksum(pentair_packet)
            print(f'Waiting to see valve with uuid: {uuid.hex("-")} reset')
            read_data = ser.read_until(expected=pentair_packet, size=1000)
            print(read_data.hex('-'))
            if pentair_packet in read_data:
                print("Valve looks to have reset")
                break
            print("Valve doesn't look reset")
            print(f'Retry {i + 1} of {num_reset_before_quit}')
        if i < num_reset_before_quit:
            print(f'Sending Packet to address: {hex(address)} uuid: {uuid.hex("-")} to keed device in bootloader mode')
            fw_packet = bytearray([0xFF, 0x0, 0xFF, 0xA5, 0x3F, address, 0xF, 0xF5, 0x2, 0x1, 0x2])
            fw_packet += uuid
            fw_packet += bytearray([0x1])
            self._calculate_and_append_checksum(fw_packet)
            ser.write(fw_mode_packet)
            ser.flush()
            time.sleep(0.5)
